...
This is the canonical publish procedure for `pi-review`.

## Scope

- Release channel: **latest** only (stable tags `vX.Y.Z`).
- Publish target: npm package **`pi-review`**.
...
6. **Post-release verification**
   - `npm view pi-review version dist-tags --json`
   - `npm view` may lag due cache. Verify with registry data:
     ```bash
     curl -s https://registry.npmjs.org/pi-review \
...
- If a bad version is published:
  1. Deprecate bad version:
     - `npm deprecate pi-review@X.Y.Z "broken release; upgrade to >=X.Y.Z+1"`
